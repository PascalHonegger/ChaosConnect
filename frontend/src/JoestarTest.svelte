<script lang="ts">
    ...
    import { newEmpty } from "./JoestarClient";

    export let client: ChaosConnectServiceClient;
    let gameUpdates: any[] = [];
    // cast is workaround for https://github.com/grpc/grpc-web/issues/950
    const updateStream = client.getGameUpdates(newEmpty()) as ClientReadableStream<GameUpdateEvent>;
    updateStream.on('data', d => gameUpdates = [...gameUpdates, d]);
    updateStream.on('error', d => gameUpdates = [...gameUpdates, d.message]);
    updateStream.on('end', () => gameUpdates = [...gameUpdates, 'session ended']);
</script>
